...
--series 20000 \
--tick 1s \
test_13_input,segment=EQU,partition=P1,lc=192.168.1.1 latNs=500i
assert_ran_ok "influx-stress"

echo "###################################################################################################"
curl -i -XPOST ${INFLUXDB_URL}/query?pretty=true --data-urlencode 'q=SELECT count(*) from "stress"."rp_stress"."test_13"'
assert_ran_ok "SELECT count(*) from \"stress\".\"rp_stress\".\"test_13\" (see log file)"
